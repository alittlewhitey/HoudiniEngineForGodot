...
class HEGSettings: public godot::Object{
    GDCLASS(HEGSettings,godot::Object)
    friend class HEGCenter;
    friend void initialize_module(godot::ModuleInitializationLevel);
    static void _bind_methods(){
        godot::ClassDB::bind_method(godot::D_METHOD("_init"),&HEGSettings::_init);
        godot::ClassDB::bind_method(godot::D_METHOD("_settings_changed"),&HEGSettings::_settings_changed);
        godot::ClassDB::bind_method(godot::D_METHOD("set_logFilePath","path"),&HEGSettings::set_logFilePath);
    }
    ...
    void _init_settings(){
        ...
        addSetting("houdini/config/hapiLib",HAPILibType::HAPIL,godot::Variant::INT,godot::PROPERTY_HINT_ENUM,"HAPIL:0,HEGHapi:1");
    ...
    void initHoudini(){
        ...
        if(useEnvLibPath)
            hapiLib = HoudiniEnginePlatform::LoadLibHAPIL(libType == HEGHapi);
        else 
            hapiLib = HoudiniEnginePlatform::LoadLibHAPIL(libType == HEGHapi, houdiniLibPath);
        if(hapiLib != nullptr){
...
public:
    ...
    static HEGSettings* get_singleton(){
        static HEGSettings* singleton = nullptr;
        if(!singleton)
            singleton = memnew(HEGSettings());
        return singleton;
    }
    enum HAPILibType{
        HAPIL = 0,
